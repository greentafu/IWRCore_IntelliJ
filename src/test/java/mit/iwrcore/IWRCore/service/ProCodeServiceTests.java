...
public class ProCodeServiceTests {
    ...
    public void insertProMCodeTest(){
        ProMDTO mdto= ProMDTO.builder().Mname("브레이크 시스템").proL(proCodeService.ProLdtoToEntity(proCodeService.findProL(1L))).build();
    ...
    public void updateProSCodeTes11t(){
        ProSDTO sdto= ProSDTO.builder().Sname("브레이크 캘리퍼 및 패드").proM(proCodeService.ProMdtoToEntity(proCodeService.findProM(5L))).build();
    ...
    public void updateProSCodeTest(){
        ProSDTO sdto= ProSDTO.builder().proScode(2L).Sname("프레임").proM(proCodeService.proMdtoToEntity(proCodeService.findProM(1L))).build();
        ProS proS =proCodeService.proSdtoToEntity(sdto);
        sCodeRepository.save(proS);
    }
    // Scode 삭제
    ...
    public void selectboxProLTest(){
        ProSDTO sdto=null;
//        PartMDTO mdto=null;
        ProMDTO mdto=ProMDTO.builder().proMcode(1L).Mname("프레임 뭐시기").proL(proCodeService.proLdtoToEntity(proCodeService.findProL(1L))).build();
